...
export default async function OpportunitiesPage() {
  ...
  const placementStages = milestones
    .filter((m: any) => m.category === 'internships' || m.category === 'industry placements')
    .map((m: any) => ({
      label: m.title,
      status: m.status,
      detail: m.impact_description || m.evidence_link || "Placement milestone details"
    }));
            ...
            <div className="grid gap-4 lg:grid-cols-3">
              <div className="rounded-xl border bg-background/80 p-4">
                ...
                <p className="mt-3 font-semibold">
                  {profile?.program || "Public-sector analytics track"}
                </p>
              </div>
              <div className="rounded-xl border bg-background/80 p-4">
                <p className="text-xs uppercase tracking-[0.24em] text-muted-foreground">
                ...
                <p className="mt-3 text-3xl font-bold">{profile?.placement_score || 0}%</p>
