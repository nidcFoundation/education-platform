...
const requiredDocuments: { type: DocumentType; label: string; description: string; required: boolean }[] = [
    { type: "transcript", label: "Academic Transcripts / WAEC Result", description: "Certified copy of your final secondary school results. Must include all subjects and grades.", required: true },
    { type: "id", label: "Government-Issued ID", description: "National ID card, International Passport, or Birth Certificate. Must be clearly legible.", required: true },
    { type: "reference_letter", label: "Reference Letter 1 (Academic)", description: "Letter from a teacher, lecturer, or school principal. Must be on official letterhead.", required: true },
    { type: "reference_letter", label: "Reference Letter 2 (Community)", description: "Letter from a community or civic leader (not a family member). Must be on official letterhead.", required: true },
            ...
            <div className="max-w-3xl mx-auto space-y-6">
                ...
                <div className="flex items-start gap-3 p-4 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-800">
                    ...
                    <div>
                        ...
                        <p>Accepted formats: PDF, JPG, PNG. Maximum file size: 5MB per document. All documents must be certified true copies where required.</p>
                    </div>
                </div>
                            ...
                            <Card key={i} className={`border ${uploaded ? "border-emerald-200/60" : "border-border/50"}`}>
                                ...
                                <CardContent className="p-4">
                                    {uploaded ? (
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                {uploaded.status === "rejected" && (
                                                    <p className="text-xs text-red-600 font-medium mt-1 flex items-center gap-1">
                                                        <AlertCircle className="h-3 w-3" /> Document rejected — please re-upload a clearer version
                                                    </p>
                                                )}
                                            </div>
                                        ...
                                        <div className="border-2 border-dashed border-muted-foreground/20 hover:border-primary/40 rounded-xl p-8 text-center transition-colors cursor-pointer group">
                    ...
                    <CardContent className="p-5">
                        <div className="flex flex-wrap gap-4 text-sm">
                            <div className="flex items-center gap-2">
                                ...
                                <span>{mockDocuments.filter(d => d.status === "verified").length} Verified</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <Clock className="h-4 w-4 text-amber-500" />
                                <span>{mockDocuments.filter(d => d.status === "pending").length} Under Review</span>
                            </div>
                            <div className="flex items-center gap-2 text-muted-foreground">
                                <Upload className="h-4 w-4" />
                                <span>{requiredDocuments.filter(d => d.required).length - mockDocuments.length} Still Needed</span>
                            </div>
                        </div>
                    </CardContent>
                </Card>
